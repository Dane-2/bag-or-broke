...
function InvestmentLog({ investments, setInvestments, setCash }) {
  const handleSell = (idx) => {
    ...
    alert(`${investment.card} sold for $${investment.newValue.toLocaleString()}`);
  };
      ...
      {investments.length === 0 ? (
        ...
        <ul className="space-y-1 text-sm text-gray-700">
          {investments.map((inv, idx) => {
            const roi = Math.round(((inv.newValue - inv.cost) / inv.cost) * 100);
            return (
              <li
                key={idx}
              ...
              >
                <span>
                  {inv.card} → ROI: {roi}% → Current Value: ${inv.newValue.toLocaleString()}
                </span>
                <button
